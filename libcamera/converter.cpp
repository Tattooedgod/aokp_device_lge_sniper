...
void SRGGB10_to_yuv420(unsigned char* src, unsigned char* dst, int width, int height)
{
    int r, g, b;
    int g1, g2;
    int y, u, v;
    ...
    unsigned char* uplane = dst + ((width*2)*(height*2));
    unsigned char* vplane = uplane + (width*height);

    for (unsigned int iy = 0; iy < height; iy += 2) {
        for (unsigned int ix = 0; ix < width; ix += 2) {
            r  = (src[iy*width+ix+0] << 8) | src[iy*width+ix+1];
            g1 = (src[iy*width+ix+2] << 8) | src[iy*width+ix+3];
            g2 = (src[(iy+1)*width+ix+0] << 8) | src[(iy+1)*width+ix+1];
            b  = (src[(iy+1)*width+ix+2] << 8) | src[(iy+1)*width+ix+3];
            g = (g1+g2)/2;
            ...
            yplane[iy*width+ix+0] = y;
            yplane[iy*width+ix+1] = y;
            yplane[(iy+1)*width+ix+0] = y;
            yplane[(iy+1)*width+ix+1] = y;
            uplane[iy*width+ix] = u;
            vplane[iy*width+ix] = v;
        }
    }
}
