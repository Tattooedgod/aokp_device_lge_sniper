...
int
V4L2Camera::Open(const char *device)
{
    int ret = 0;
    int ccdc_fd, tvp_fd;
...
int
V4L2Camera::Open_media_device(const char *device)
{
    int ret = 0;
    int index = 0;
    int i;
...
int
V4L2Camera::Configure(int width, int height, int pixelformat, int fps, int cam_mode)
{
    int ret = 0;
...
int
V4L2Camera::setFramerate(int framerate, int cam_mode)
{
    int ret;
    struct v4l2_streamparm parm;
...
int
V4L2Camera::BufferMap(int cam_mode)
{
    int ret, mMaxBuffers;
...
void
V4L2Camera::reset_links(const char *device)
{
    struct media_link_desc link;
    struct media_links_enum links;
    int ret, index, i;
...
void
V4L2Camera::Uninit(int cam_mode)
{
    int ret, mMaxBuffers;
    if (m_flag_init) {
        //dhiru1602: If the camera is in Capture Mode, use only a Single Buffer
...
void *
V4L2Camera::GrabRecordFrame(int& index)
{
    int ret;
...
void
V4L2Camera::ReleaseRecordFrame(int index)
{
    int ret;
...
void
V4L2Camera::GrabRawFrame(void *previewBuffer, unsigned int width, unsigned int height)
{
    int ret = 0;
    int DQcount = 0;
...
#ifdef _OMAP_RESIZER_
        ret = OMAPResizerConvert(videoIn->resizeHandle,
...
camera_memory_t *
V4L2Camera::GrabJpegFrame(camera_request_memory mRequestMemory, unsigned long &mfilesize, bool IsFrontCam)
{
...
#ifdef V4L2_CID_JPEG_SIZE
    bool ManualEncode = IsFrontCam;
#else
    bool ManualEncode = true;
#endif
...
int
V4L2Camera::GetThumbNailOffset()
{
#ifdef V4L2_CID_FW_THUMBNAIL_OFFSET
    struct v4l2_control vc;
...
int
V4L2Camera::GetYUVOffset()
{
#ifdef V4L2_CID_FW_YUV_OFFSET
    struct v4l2_control vc;
...
int
V4L2Camera::GetThumbNailDataSize()
{
#ifdef V4L2_CID_THUMBNAIL_SIZE
    struct v4l2_control vc;
...
int
V4L2Camera::GetJPEG_Capture_Width()
{
#ifdef V4L2_CID_JPEG_CAPTURE_WIDTH
    struct v4l2_control vc;
...
int
V4L2Camera::GetJPEG_Capture_Height()
{
#ifdef V4L2_CID_JPEG_CAPTURE_HEIGHT
    struct v4l2_control vc;
...
int
V4L2Camera::GetCamera_version()
{
#ifdef V4L2_CID_FW_VERSION
    struct v4l2_control vc;
...
void
V4L2Camera::getExifInfoFromDriver(V4L2ExifObj * exifobj)
{
#ifdef VIDIOC_G_EXIF
...
camera_memory_t *
V4L2Camera::CreateJpegFromBuffer(void *rawBuffer, camera_request_memory mRequestMemory)
{
...
unsigned long
V4L2Camera::saveYUYVtoJPEG(unsigned char *inputBuffer, int width, int height, unsigned char *&outbuffer, int quality)
{
...
static int
v4l2_s_ctrl(int fp, unsigned int id, unsigned int value)
{
...
static int
v4l2_g_ctrl(int fp, unsigned int id)
{
    struct v4l2_control ctrl;
    int ret;
...
int
V4L2Camera::getAutoFocusResult(void)
{
...
int
V4L2Camera::cancelAutofocus(void)
{
    LOGV("%s :", __func__);
...
// Zoom Setting
int
V4L2Camera::setZoom(int zoom_level)
{
...
int
V4L2Camera::setWhiteBalance(int white_balance)
{
    LOGV("%s(white_balance(%d))", __func__, white_balance);

#if defined(WHITE_BALANCE_BASE) && defined(WHITE_BALANCE_MAX)
...
int
V4L2Camera::getWhiteBalance()
{
    return (mWhiteBalance);
}

int
V4L2Camera::setSceneMode(int scene_mode)
{
    LOGV("%s(scene_mode(%d))", __func__, scene_mode);

#ifdef V4L2_CID_SCENE
...
int
V4L2Camera::getSceneMode()
{
    return (mSceneMode);
}

int
V4L2Camera::setFocusMode(int focus_mode)
{
    LOGV("%s(focus_mode(%d))", __func__, focus_mode);

#ifdef V4L2_CID_FOCUS_MODE
...
int
V4L2Camera::setAEAWBLockUnlock(int ae_lockunlock, int awb_lockunlock)
{
#ifdef V4L2_CID_CAMERA_AE_AWB_LOCKUNLOCK
    int ae_awb_status = 0;
...
int
V4L2Camera::SetCameraFlip(bool isCapture)
{
#ifdef V4L2_CID_FLIP
    struct v4l2_control vc;
    vc.id = V4L2_CID_FLIP;
...
int
V4L2Camera::setExifOrientationInfo(int orientationInfo)
{
...
// ===============================================================================================
int
V4L2Camera::setISO(int iso_value)
{
#ifdef V4L2_CID_ISO
...
// ===============================================================================================
int
V4L2Camera::setMetering(int metering_value)
{
#ifdef V4L2_CID_PHOTOMETRY
...
// ===============================================================================================
int
V4L2Camera::setContrast(int contrast_value)
{
    LOGV("%s(contrast_value(%d))", __func__, contrast_value);

#if defined(CONTRAST_MINUS_2) && defined(CONTRAST_MAX)
...
// ===============================================================================================
int
V4L2Camera::setSharpness(int sharpness_value)
{
    LOGV("%s(sharpness_value(%d))", __func__, sharpness_value);

#if defined(SHARPNESS_MINUS_2) && defined(SHARPNESS_MAX)
...
// ===============================================================================================
int
V4L2Camera::setSaturation(int saturation_value)
{
#if defined(SATURATION_MINUS_2) && defined(SATURATION_MAX)
...
int
V4L2Camera::setGain(int gain)
{
#if defined(V4L2_CID_GAIN)
    if (videoIn->isStreaming) {
        if (v4l2_s_ctrl
            (camHandle, V4L2_CID_GAIN, gain) < 0) {
            LOGE("ERR(%s):Fail on V4L2_CID_GAIN", __func__);
            return -1;
        }
    }
#endif
...
int
V4L2Camera::setExposure(int exposure)
{
#if defined(V4L2_CID_EXPOSURE)
    if (videoIn->isStreaming) {
        if (v4l2_s_ctrl
            (camHandle, V4L2_CID_EXPOSURE, exposure) < 0) {
            LOGE("ERR(%s):Fail on V4L2_CID_EXPOSURE", __func__);
            return -1;
        }
    }
#endif
...
int
V4L2Camera::setBrightness(int brightness)
{
    ...
    //XXX: userspace range (-4,+4), kernel range (0..30)
    LOGE("%s: setBrightness: %d", __func__, brightness);
    brightness = (brightness+4)*3;
...
// ===============================================================================================
int
V4L2Camera::getOrientation()
{
    return (m_exif_orientation);
}

void
V4L2Camera::convert(unsigned char *buf, unsigned char *rgb, int width, int height)
{
    int x, y, z = 0;
    int blocks;
