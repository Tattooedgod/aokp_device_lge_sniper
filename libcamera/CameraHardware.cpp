...
#define VIDEO_DEVICE_FFC    "/dev/video0"
#define VIDEO_DEVICE_BFC    "/dev/video5"
#define MEDIA_DEVICE        "/dev/media0"
#define PREVIEW_WIDTH       640
#define PREVIEW_HEIGHT      480
#define PIXEL_FORMAT       V4L2_PIX_FMT_YUYV
//Define Cameras
#define CAMERA_BF 1             //Back Camera
#define CAMERA_FF 0
    ...
    mRecordingEnabled(false)
{
    ...
    if (CameraID == CAMERA_FF) {
        mCamera->Open(VIDEO_DEVICE_FFC);
    }
    else {
        mCamera->Open(VIDEO_DEVICE_BFC);
    }
...
void
CameraHardware::initDefaultParameters(int CameraID)
{
    CameraParameters p;
...
int
CameraHardware::get_kernel_version()
{
...
sp<IMemoryHeap>
CameraHardware::getPreviewHeap() const
{
    LOGV("Preview Heap");
    return mPreviewHeap;
}
...
sp<IMemoryHeap>
CameraHardware::getRawHeap() const
{
    LOGV("Raw Heap");
    return mRawHeap;
}
...
void
CameraHardware::setCallbacks(camera_notify_callback notify_cb,
...
int
CameraHardware::setPreviewWindow(preview_stream_ops_t * window)
{
    int err;
    Mutex::Autolock lock(mLock);
    if (mNativeWindow)
...
void
CameraHardware::enableMsgType(int32_t msgType)
{
...
void
CameraHardware::disableMsgType(int32_t msgType)
{
...
bool
CameraHardware::msgTypeEnabled(int32_t msgType)
{
...
bool
CameraHardware::validateSize(size_t width, size_t height, const supported_resolution * supRes, size_t count)
{
    bool ret = false;
    ...
    if (!previewStopped) {
        ...
        if (mNativeWindow != NULL) {
            ...
            if (0 == mapper.lock((buffer_handle_t) * hndl2hndl, GRALLOC_USAGE_SW_WRITE_OFTEN, bounds, &dst)) {
                ...
                //XXX: teamhacksung does not check CAMERA_MSG_VIDEO_FRAME, gabwerkz does
...
bool
CameraHardware::recordingEnabled()
{
    return mRecordingEnabled;
}

void
CameraHardware::releaseRecordingFrame(const void *opaque)
{
    int i;
...
int
CameraHardware::beginAutoFocusThread(void *cookie)
{
...
int
CameraHardware::autoFocusThread()
{
    int count = 0;
    int af_status = 0;
...
int
CameraHardware::beginPictureThread(void *cookie)
{
    CameraHardware *c = (CameraHardware *) cookie;
...
status_t
CameraHardware::dump(int fd, const Vector<String16>& args) const
{
    return NO_ERROR;
}

status_t
CameraHardware::setParameters(const CameraParameters & params)
{
    int width = 0;
    int height = 0;
    int framerate = 0;
...
CameraParameters
CameraHardware::getParameters() const
{
...
status_t
CameraHardware::sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    return BAD_VALUE;
}
...
void
CameraHardware::CreateExif(unsigned char *pInThumbnailData,
                           int Inthumbsize,
                           ...
                           int &OutExifSize, int flag)
{
    ...
    strcpy((char *) &ExifInfo.maker, "LGE");
    strcpy((char *) &ExifInfo.model, "Sniper");
    ...
    if (mCameraID == CAMERA_BF) {
        ...
        if (mCamera->getSceneMode() <= 1) {
            ...
            ExifInfo.saturation =
                convertToExifLMH(mCamera->getSaturation(), 2);
...
void
CameraHardware::convertFromDecimalToGPSFormat(double arg1, int &arg2, int &arg3, double &arg4)
{
...
int
CameraHardware::convertToExifLMH(int value, int key)
{
    const int NORMAL = 0;
    const int LOW = 1;
    const int HIGH = 2;
...
double
CameraHardware::getGPSLatitude() const
{
...
double
CameraHardware::getGPSLongitude() const
{
...
double
CameraHardware::getGPSAltitude() const
{
...
void
CameraHardware::setSkipFrame(int frame)
{
